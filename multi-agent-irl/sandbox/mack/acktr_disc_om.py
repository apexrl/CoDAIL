...
class Model(object):
    ...
    def __init__(self, policy, oppo_policy, ob_space, ac_space, nenvs, total_timesteps, nprocs=2, nsteps=200,
        ...
        def step(ob, av, *_args, **_kwargs): # av is a_{t-1} (joint policy)
            a, v, s = [], [], []
...
class Runner(object):
    ...
    def run(self):
        ...
        for k in range(self.num_agents):
            ...
            mb_rewards[k] = np.asarray(mb_rewards[k], dtype=np.float32).swapaxes(1, 0) # (#runners, batchsize)
        ...
        for k in range(self.num_agents):
            for n, (rewards, dones, value) in enumerate(zip(mb_rewards[k], mb_dones[k], last_values[k].tolist())):
                ...
                mb_returns[k][n] = rewards # n = the number of runners
        ...
        return mb_obs, mb_states, mb_returns, mb_masks, mb_actions, mb_values#, mb_episode_r


def learn(policy, oppo_policy, env, seed, total_timesteps=int(40e6), gamma=0.95, lam=0.92, log_interval=1, nprocs=32, nsteps=20,
          nstack=1, ent_coef=0.00, vf_coef=0.5, vf_fisher_coef=1.0, lr=0.25, max_grad_norm=0.5,
          kfac_clip=0.001, save_interval=50, lrschedule='linear', identical=None, load=False, epoch=55000, path=None):
    ...
    make_model = lambda: Model(policy, oppo_policy, ob_space, ac_space, nenvs, total_timesteps, nprocs=nprocs, nsteps
    ...
    print("-------------------------------")
    print(total_timesteps//nbatch+1)
    print("-------------------------------")
    for update in range(1, total_timesteps//nbatch+1):
        # obs, states, rewards, masks, actions, values, episode_r = runner.run()
        ...
        if update % log_interval == 0 or update == 1:
            ...
            logger.record_tabular("nupdates", update+epoch)
            logger.record_tabular("total_timesteps", (update+epoch)*nbatch)
            ...
            for k in range(model.num_agents):
                ...
                # logger.record_tabular("reward %d" % k, float(np.mean(episode_r[k])))
            # logger.record_tabular("sum_reward", float(np.sum(episode_r)))
            logger.dump_tabular()

        if save_interval and (update % save_interval == 0 or update == 1) and logger.get_dir():
            savepath = osp.join(logger.get_dir(), 'checkpoint%.5i' % (update+epoch))
