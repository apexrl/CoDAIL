...
from irl.mack.ncdail import learn
...
@click.option('--expert_path', type=click.STRING,
              default='./results/mack_om/simple_push/l-0.1-b-1000/seed-1/checkpoint55000-200tra.pkl')
...
def main(logdir, env, expert_path, seed, traj_limitation, ret_threshold, dis_lr, disc_type, bc_iters, d_iters):
    ...
    for env_id, seed, lr, batch_size in itertools.product(env_ids, seeds, lrs, batch_sizes):
        logdir = ldir + '/ncdail/' + env_id + '/' + disc_type + '/s-{}/l-{}-b-{}-d-{}-c-{}/seed-{}'.format(
